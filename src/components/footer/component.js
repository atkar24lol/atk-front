...
const Footer = () => {
    ...
    return <div
        className='w-full bg-[#0072BC] text-[#fff] min-h-[500px] flex flex-col px-[82px] md:px-0 md:justify-center sm:px-[20px] sm:justify-center pt-[50px] pb-[136px]'>
        <div className='flex gap-5 md:gap-3 sm:gap-3 items-center md:justify-center sm:justify-center pb-[46px]'>
            ...
            <span className='text-[20px] md:text-[18px] sm:text-[14px] text-[#fff] font-[600]'
                  dangerouslySetInnerHTML={{__html: dict?.footer?.slogan}}></span>
        </div>

        <div>
            <div className='flex gap-[80px] justify-between md:justify-center sm:justify-center flex-wrap'>
                <div
                    className='flex gap-[80px] md:gap-[50px] sm:gap-[50px] flex-wrap md:justify-center md:text-center sm:justify-start sm:pl-5'>
                    <div className='flex md:px-4 sm:px-3 flex-col gap-[10px] cursor-pointer'>{footerListOne?.map(({
                                                                                                                      name,
                                                                                                                      link
                                                                                                                  }, index) => (
                        <p key={index} onClick={() => router.push(link)}
                           className='font-[400] text-[16px] text-[#fff] hover:underline'>{name}</p>
                    ))}
                    </div>

                    <div className='flex md:px-4 sm:px-3 flex-col gap-[10px] cursor-pointer'>{footerListTwo?.map(({
                                                                                                                      name,
                                                                                                                      link
                                                                                                                  }) => (
                ...
                <div className='flex flex-col justify-between items-center gap-[11px] md:px-4 sm:px-3'>
                    <img onClick={() => router.push(`http://cdo.atk.kg/`)} src='/logo-dark.png'
                         className='w-[150px] h-[45px] cursor-pointer'/>
                    <p onClick={() => router.push(`http://cdo.atk.kg/`)}
                       className='cursor-pointer font-[400] text-[14px] text-[#fff]'>{dict?.footer?.list?.educationalPortal}</p>
            ...
            <div className='flex justify-around items-center w-auto gap-5 min-w-[100px]'>
                <a href='https://www.instagram.com/atkbishkek/'>
                    <img src='/insta-footer-icon.svg'
                         className='w-[32px] h-[32px] transition-[1s] hover:transition-[1s] cursor-pointer hover:text-[#922c81] fill-[#922c81]'/>
                </a>
